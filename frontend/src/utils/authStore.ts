...
function saveAccounts(accounts: UserAccount[]): void {
  ...
  // Background cloud sync for hybrid enterprise persistence
  setTimeout(() => {
    accounts.forEach((acc) => {
      syncCloudAccount(acc).catch(() => {});
    });
  }, 10);
}
...
export async function ensureSuperAdmin(): Promise<void> {
  // Try pulling global accounts from cloud/MongoDB Atlas first
  try {
    ...
    if (cloudAccounts && Array.isArray(cloudAccounts) && cloudAccounts.length > 0) {
      ...
      cloudAccounts.forEach((a) => mergedMap.set(a.id, a));
      const mergedList = Array.from(mergedMap.values());
      localStorage.setItem(ACCOUNTS_KEY, JSON.stringify(mergedList));
    }
  ...
  await syncCloudAccount(superAdmin).catch(() => {});
}
...
export function adminDeleteUser(adminSession: Session, targetId: string): void {
  ...
  deleteCloudAccount(targetId).catch(() => {});
}
...
export async function saveVault(userId: string, password: string, data: unknown): Promise<void> {
  ...
  await saveCloudVault(userId, encrypted).catch(() => {});
}

export async function loadVault<T>(userId: string, password: string, fallback: T): Promise<T> {
  let blob = localStorage.getItem(VAULT_PFX + userId);
  if (!blob) {
    try {
      const cloudBlob = await fetchCloudVault(userId);
      if (cloudBlob) {
        blob = cloudBlob;
        localStorage.setItem(VAULT_PFX + userId, cloudBlob);
      }
